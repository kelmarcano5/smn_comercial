package domain.repgen.smn_proceso.smn_control_renovacion;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("crn_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_afiliado_id");
			if (d != null)
			{
			name  = "b_afiliation";
			label = "${lbl:b_afiliation}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("crn_fecha_renovacion");
			if (d != null)
			{
			name  = "b_renovation_date";
			label = "${lbl:b_renovation_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("crn_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
