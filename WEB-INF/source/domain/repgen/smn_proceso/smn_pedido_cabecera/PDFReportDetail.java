package domain.repgen.smn_proceso.smn_pedido_cabecera;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_relacionado_id");
			if (d != null)
			{
			name  = "b_related";
			label = "${lbl:b_related}";
		...
		d = rs.getString("res_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("res_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
