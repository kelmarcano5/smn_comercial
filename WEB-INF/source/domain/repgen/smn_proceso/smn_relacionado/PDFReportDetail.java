package domain.repgen.smn_proceso.smn_relacionado;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("rel_auxiliar_rf");
			if (d != null)
			{
			name  = "b_auxiliary";
		...
		d = rs.getString("rel_auxiliar_rf_name");
			if (d != null)
			{
			name  = "b_auxiliary";
			label = "${lbl:b_auxiliary}";
		...
		d = rs.getString("rel_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_afiliados_id");
			if (d != null)
			{
			name  = "b_afiliate";
			label = "${lbl:b_afiliate}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rel_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
