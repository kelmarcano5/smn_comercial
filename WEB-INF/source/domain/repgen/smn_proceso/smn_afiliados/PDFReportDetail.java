package domain.repgen.smn_proceso.smn_afiliados;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("afi_auxiliar_rf");
			if (d != null)
			{
			name  = "b_auxiliary";
		...
		d = rs.getString("afi_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_servicio_id");
			if (d != null)
			{
			name  = "b_services";
		...
		d = rs.getString("afi_fecha_registro_end");
			if (d != null)
			{
			name  = "${lbl:b_store_date}";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("afi_fecha_inicio");
			if (d != null)
			{
			name  = "b_date_init";
			label = "${lbl:b_date_init}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_servicio_id_name");
			if (d != null)
			{
			name  = "b_services";
			label = "${lbl:b_services}";
		...
		d = rs.getString("afi_auxiliar_rf_name");
			if (d != null)
			{
			name  = "b_auxiliary";
			label = "${lbl:b_auxiliary}";
