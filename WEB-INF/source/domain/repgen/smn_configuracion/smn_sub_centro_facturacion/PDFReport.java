package domain.repgen.smn_configuracion.smn_sub_centro_facturacion;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("scf_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("scf_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_centro_facturacion_id_name");
			if (d != null)
			{
			name  = "b_centro_facturacion";
			label = "${lbl:b_centro_facturacion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_centro_facturacion_id");
			if (d != null)
			{
			name  = "b_centro_facturacion";
			label = "${lbl:b_centro_facturacion}";
