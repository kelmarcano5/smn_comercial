package domain.repgen.smn_configuracion.smn_cliente;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("cli_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_clasificacion_cliente_id_name");
			if (d != null)
			{
			name  = "b_clasificacion_cliente";
			label = "${lbl:b_clasificacion_cliente}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cli_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_clase_auxiliar_rf_name");
			if (d != null)
			{
			name  = "b_auxiliary_classes";
			label = "${lbl:b_auxiliary_classes}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
