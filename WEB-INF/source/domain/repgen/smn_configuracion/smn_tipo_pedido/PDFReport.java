package domain.repgen.smn_configuracion.smn_tipo_pedido;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("tpp_estatus");
			if (d != null)
			{
			name  = "${lbl:s_profile_lang}";
		...
		d = rs.getString("tpp_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("tpp_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("tpp_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
