...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("dcf_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("smn_documentos_generales_rf_name");
			if (d != null)
			{
			name  = "b_general_documents";
			label = "${lbl:b_general_documents}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dcf_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_tipo_documentos_fact_id");
			if (d != null)
			{
			name  = "b_tipo_documento_factura";
			label = "${lbl:b_tipo_documento_factura}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dcf_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_tipo_documentos_fact_id_name");
			if (d != null)
			{
			name  = "b_tipo_documento_factura";
			label = "${lbl:b_tipo_documento_factura}";
