package domain.repgen.smn_configuracion.smn_documento_grp_titulo_imp;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("rdi_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_grupo_titulo_impresion_rf");
			if (d != null)
			{
			name  = "b_group_tittle_print";
			label = "${lbl:b_group_tittle_print}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rdi_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_grupo_titulo_impresion_rf_name");
			if (d != null)
			{
			name  = "b_group_tittle_print";
			label = "${lbl:b_group_tittle_print}";
