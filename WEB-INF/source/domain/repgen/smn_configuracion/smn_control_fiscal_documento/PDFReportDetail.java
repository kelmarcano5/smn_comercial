package domain.repgen.smn_configuracion.smn_control_fiscal_documento;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("cfd_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("cfd_numero_documento_fiscal_final");
			if (d != null)
			{
			name  = "b_doc_num_fis_fin";
			label = "${lbl:b_doc_num_fis_fin}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cfd_ultimo_numero_fiscal_usado");
			if (d != null)
			{
			name  = "b_num_ult_fis_usa";
			label = "${lbl:b_num_ult_fis_usa}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cfd_numero_documento_fiscal_inicial");
			if (d != null)
			{
			name  = "b_doc_num_fis_ini";
			label = "${lbl:b_doc_num_fis_ini}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cfd_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
