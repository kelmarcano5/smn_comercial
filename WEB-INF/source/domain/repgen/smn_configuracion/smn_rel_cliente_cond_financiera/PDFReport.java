package domain.repgen.smn_configuracion.smn_rel_cliente_cond_financiera;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_condicion_financiera_rf_name");
			if (d != null)
			{
			name  = "b_condition_financiery";
			label = "${lbl:b_condition_financiery}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_condicion_financiera_rf");
			if (d != null)
			{
			name  = "b_condition_financiery";
			label = "${lbl:b_condition_financiery}";
		...
		d = rs.getString("rcc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("rcc_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
