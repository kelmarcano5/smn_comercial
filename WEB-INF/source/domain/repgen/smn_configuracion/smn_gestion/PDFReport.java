package domain.repgen.smn_configuracion.smn_gestion;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("ges_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("ges_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_accion_id_name");
			if (d != null)
			{
			name  = "b_action";
			label = "${lbl:b_action}";
		...
		d = rs.getString("ges_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
