package domain.repgen.smn_configuracion.smn_detalle_documento_titulo_imp;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_item_servicio_rf");
			if (d != null)
			{
			name  = "b_document";
		...
		d = rs.getString("smn_documento_grp_titulo_imp_id_name");
			if (d != null)
			{
			name  = "b_relacion_documento_titulo_impresion";
			label = "${lbl:b_relacion_documento_titulo_impresion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dgi_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_item_servicio_rf_name");
			if (d != null)
			{
			name  = "b_document";
			label = "${lbl:b_document}";
		...
		d = rs.getString("dgi_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_documento_grp_titulo_imp_id");
			if (d != null)
			{
			name  = "b_relacion_documento_titulo_impresion";
			label = "${lbl:b_relacion_documento_titulo_impresion}";
