package domain.repgen.smn_configuracion.smn_rel_cliente_area_interes;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_area_interes_id_name");
			if (d != null)
			{
			name  = "b_areas_interes";
			label = "${lbl:b_areas_interes}";
		...
		d = rs.getString("rca_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("rca_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_area_interes_id");
			if (d != null)
			{
			name  = "b_areas_interes";
			label = "${lbl:b_areas_interes}";
