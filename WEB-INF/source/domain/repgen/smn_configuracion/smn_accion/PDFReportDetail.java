package domain.repgen.smn_configuracion.smn_accion;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("acc_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("acc_paso_rf_name");
			if (d != null)
			{
			name  = "b_pass";
			label = "${lbl:b_pass}";
		...
		d = rs.getString("acc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
