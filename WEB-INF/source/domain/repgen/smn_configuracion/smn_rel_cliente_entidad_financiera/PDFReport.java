package domain.repgen.smn_configuracion.smn_rel_cliente_entidad_financiera;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_tipo_cuenta_banco_rf");
			if (d != null)
			{
			name  = "b_types_of_bank_accounts";
			label = "${lbl:b_types_of_bank_accounts}";
		...
		d = rs.getString("smn_entidad_financiera_rf");
			if (d != null)
			{
			name  = "b_financial_entities";
			label = "${lbl:b_financial_entities}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_cliente_id_name");
			if (d != null)
			{
			name  = "b_customer";
			label = "${lbl:b_customer}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_cuenta_banco_rf_name");
			if (d != null)
			{
			name  = "b_types_of_bank_accounts";
			label = "${lbl:b_types_of_bank_accounts}";
		...
		d = rs.getString("cef_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("cef_fecha_registro_ini");
			if (d != null)
			{
