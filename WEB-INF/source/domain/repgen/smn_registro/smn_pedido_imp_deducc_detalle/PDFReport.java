package domain.repgen.smn_registro.smn_pedido_imp_deducc_detalle;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("pid_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_pedido_detalle_id_name");
			if (d != null)
			{
			name  = "b_header_order_detail";
			label = "${lbl:b_header_order_detail}";
		...
		d = rs.getString("pid_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
