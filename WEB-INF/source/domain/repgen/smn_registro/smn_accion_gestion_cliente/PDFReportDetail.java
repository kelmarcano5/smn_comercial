package domain.repgen.smn_registro.smn_accion_gestion_cliente;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("ago_estatus_proceso");
			if (d != null)
			{
		...
		d = rs.getString("smn_rel_oferta_cliente_id");
			if (d != null)
			{
			name  = "b_rel_customer_offer";
			label = "${lbl:b_rel_customer_offer}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
