package domain.repgen.smn_registro.smn_envio_oferta;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("eof_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("smn_clientes_area_interes_id");
			if (d != null)
			{
			name  = "b_customer_interesting_area";
			label = "${lbl:b_customer_interesting_area}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("eof_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_oferta_id_name");
			if (d != null)
			{
			name  = "b_offer";
			label = "${lbl:b_offer}";
		...
		d = rs.getString("eof_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_clasificacion_cliente_id");
			if (d != null)
			{
			name  = "b_customer_clasification";
			label = "${lbl:b_customer_clasification}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
