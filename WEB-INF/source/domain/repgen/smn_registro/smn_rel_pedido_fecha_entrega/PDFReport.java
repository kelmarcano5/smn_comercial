package domain.repgen.smn_registro.smn_rel_pedido_fecha_entrega;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_rel_auxiliar_direccion");
			if (d != null)
			{
			name  = "b_delivery_address";
			label = "${lbl:b_delivery_address}";
		...
		d = rs.getString("smn_pedido_detalle_id");
			if (d != null)
			{
			name  = "b_header_order_detail";
			label = "${lbl:b_header_order_detail}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("pfe_fecha_entrega");
			if (d != null)
			{
			name  = "b_delivery_date";
			label = "${lbl:b_delivery_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_rel_auxiliar_direccion_name");
			if (d != null)
			{
			name  = "b_delivery_address";
			label = "${lbl:b_delivery_address}";
