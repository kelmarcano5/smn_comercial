...
public class GenerarPedidoIng extends GenericTransaction{
	...
	public int service(Recordset inputParams) throws Throwable{
		...
		try{
			...
			if(rsMovimientoPedido.getRecordCount() > 0) {
				...
				infolab(conn, inputParams, str,bw);
	...
	public int infolab(Connection conn, Recordset inputParams, String str, BufferedWriter bw) throws Throwable{
		int rc = 0;
		Timestamp timestamp = new Timestamp(System.currentTimeMillis());
		...
		try{
			Db db = getDb();
			str = "-----INICIO PROCESO -----"+timestamp+"----- INFOLAB -----";
			bw.write(str);
			bw.flush();
			bw.newLine();
			
			str = str + "Registro de facturas finalizado"+ " \n";
			...
			String sqlCheckMov = getSQL(getResource("sql-consultaMovimiento_caja.sql"), inputParams);
			//crea un recordset con la data obtenida
			Recordset rsMovimientoCaja = db.get(sqlCheckMov);
			if(rsMovimientoCaja.getRecordCount() > 0){
			...
			double monto = rsMovimientoCaja.getDouble("monto");
			String xml_examenes = "";
			String xml_fin = "";
			String xml_inicio ="<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>"+
						       "<infolab xmlns=\"http://www.infolab.com/schema/1.0/interfaz-infolab\">"+
						        ...
						        "<documento>"+					
						            ...
						            "<monto-documento>"+monto+"</monto-documento>"+
			...
			String sqlCheckDetalle = getSQL(getResource("sql-consultaDetalle_caja.sql"), inputParams);
