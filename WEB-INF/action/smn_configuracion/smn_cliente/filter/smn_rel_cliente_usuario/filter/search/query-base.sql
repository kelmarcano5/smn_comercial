select
	smn_comercial.smn_rel_cliente_usuario.smn_rel_cliente_usuario_id,
	smn_base.smn_auxiliar.aux_descripcion as smn_cliente_id,
	smn_base.smn_v_usuarios_2.usr_nombres ||' '|| smn_base.smn_v_usuarios_2.usr_apellidos as smn_usuario_rf
from
	smn_comercial.smn_rel_cliente_usuario
	inner join smn_base.smn_v_usuarios_2 on  smn_base.smn_v_usuarios_2.smn_usuarios_id = smn_comercial.smn_rel_cliente_usuario.smn_usuario_rf
	inner join smn_comercial.smn_cliente on smn_comercial.smn_cliente.smn_cliente_id = 	smn_comercial.smn_rel_cliente_usuario.smn_cliente_id
	inner join smn_base.smn_auxiliar on smn_base.smn_auxiliar.smn_auxiliar_id = smn_comercial.smn_cliente.smn_auxiliar_rf

where
	smn_rel_cliente_usuario_id is not null
	${filter}
order by
		smn_rel_cliente_usuario_id
