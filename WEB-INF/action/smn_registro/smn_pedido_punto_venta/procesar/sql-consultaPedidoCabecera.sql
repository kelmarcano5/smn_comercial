select
	...
	cli.aux_codigo ||' - '|| cli.aux_descripcion as smn_cliente_id,
	...
	smn_base.smn_sucursales.suc_codigo ||' - '|| smn_base.smn_sucursales.suc_nombre as smn_sucursal_rf_combo,
	smn_base.smn_areas_servicios.ase_codigo ||' - '|| smn_base.smn_areas_servicios.ase_descripcion as smn_area_servicio_rf_combo,
	smn_base.smn_unidades_servicios.uns_codigo ||' - '|| smn_base.smn_unidades_servicios.uns_descripcion as smn_unidad_servicio_rf_combo,
	...
	smn_comercial.smn_pedido_cabecera.smn_auxiliar_rf as auxiliar,
	...
	case
		when smn_base.smn_condicion_financiera.cfi_cant_dias is null then -1 else smn_base.smn_condicion_financiera.cfi_cant_dias
	end as cant_dias,
	case
		when smn_comercial.smn_pedido_cabecera.smn_cliente_id = 0 or smn_comercial.smn_pedido_cabecera.smn_cliente_id is null then comp.aux_rif else cli.aux_rif
	end as fca_rif,
	case
		when smn_comercial.smn_pedido_cabecera.smn_cliente_id = 0 or smn_comercial.smn_pedido_cabecera.smn_cliente_id is null then (select smn_base.smn_direccion.dir_descripcion from smn_base.smn_direccion where smn_base.smn_direccion.smn_direccion_id= comp.aux_direccion_rf) else (select smn_base.smn_direccion.dir_descripcion from smn_base.smn_direccion where smn_base.smn_direccion.smn_direccion_id=cli.aux_direccion_rf)
	end as fca_direccion_fiscal
from
	...
	left join smn_base.smn_entidades on smn_base.smn_entidades.smn_entidades_id = smn_comercial.smn_pedido_cabecera.smn_entidad_rf
	...
	left join smn_base.smn_auxiliar cli on cli.smn_auxiliar_id = smn_comercial.smn_cliente.smn_auxiliar_rf
	...
	left join smn_base.smn_documentos_generales on smn_base.smn_documentos_generales.smn_documentos_generales_id = smn_comercial.smn_documento.smn_documentos_generales_rf
	left join smn_base.smn_modulos on smn_base.smn_modulos.smn_modulos_id = smn_base.smn_documentos_generales.dcg_modulo_origen
...
where 
	...
	AND smn_comercial.smn_pedido_cabecera.pca_estatus = 'RE' LIMIT 1
