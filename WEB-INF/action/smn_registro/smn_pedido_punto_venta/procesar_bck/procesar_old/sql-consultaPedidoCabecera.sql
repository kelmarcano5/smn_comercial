select
	...
	case
		when smn_comercial.smn_pedido_cabecera.smn_moneda_rf is null then 0 else smn_comercial.smn_pedido_cabecera.smn_moneda_rf
...
from
	...
	left outer join smn_comercial.smn_pedido_pagadores on smn_comercial.smn_pedido_pagadores.smn_pedido_cabecera_id = smn_comercial.smn_pedido_cabecera.smn_pedido_cabecera_id
...
WHERE smn_comercial.smn_pedido_cabecera.smn_pedido_cabecera_id = ${fld:smn_pedido_cabecera_id} AND
	  pca_estatus = 'RE' 
	  --AND   	--que este pendiente
	  --pca_fecha_registro = '${def:date}' -- y que la fecha de generacion se la actual
LIMIT 1
