...
where smn_compras.smn_tipo_documento.tdc_naturaleza='RE' and smn_compras.smn_documentos.dcc_codigo like '%RQS_04%'),
	(select 'Requisicion Originada segun Ingreso:'||' '||  (select smn_comercial.smn_presupuesto.smn_ingresos_id from smn_comercial.smn_presupuesto where smn_comercial.smn_presupuesto.smn_presupuesto_id=${fld:smn_presupuesto_id}) ||' Aprobada en Fecha: '|| to_char(pre_fecha_registro, 'YYYY-MM-DD') as descripcion from smn_comercial.smn_presupuesto where smn_comercial.smn_presupuesto.smn_presupuesto_id=${fld:smn_presupuesto_id}),
	{d '${def:date}'},
	'GE',
