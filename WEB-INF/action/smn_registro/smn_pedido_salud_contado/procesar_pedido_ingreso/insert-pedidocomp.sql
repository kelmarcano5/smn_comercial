...
(
	...
	(select smn_base.smn_componentes.cmp_grupo_prestador from smn_base.smn_componentes where smn_base.smn_componentes.smn_componentes_id = ${fld:smn_componentes_id} limit 1),
	...
	${fld:smn_moneda_rf},
	${fld:smn_tasa_rf},
	(case when ${fld:pde_precio_moneda_alterna} is null then 0 else ${fld:pde_precio_moneda_alterna} end),
    (case when ${fld:monto_componente_ma} is null then 0 else ${fld:monto_componente_ma} end),
	${fld:smn_imprime},
	...
	{d '${def:date}'},
	'${def:time}',
	(select smn_comercial.smn_rel_pac_conta_gps_defecto.smn_prestador_servicio_rf from smn_comercial.smn_rel_pac_conta_gps_defecto where smn_comercial.smn_rel_pac_conta_gps_defecto.smn_grupo_prestador_rf=(select smn_base.smn_componentes.cmp_grupo_prestador from smn_base.smn_componentes where smn_base.smn_componentes.smn_componentes_id = ${fld:smn_componentes_id} limit 1))
);