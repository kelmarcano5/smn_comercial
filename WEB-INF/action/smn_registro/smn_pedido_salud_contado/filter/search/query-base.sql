select DISTINCT
	...
	case 
		when smn_comercial.smn_pedido_cabecera.smn_modulo_rf is null then 'SMN_CME' else smn_base.smn_modulos.mod_codigo
	end as modulo,
...
from
	...
	inner join smn_base.smn_modulos on smn_base.smn_modulos.smn_modulos_id = CAST(smn_comercial.smn_pedido_cabecera.smn_modulo_rf AS INTEGER)
where
	smn_pedido_cabecera_id is not null and smn_comercial.smn_pedido_cabecera.smn_entidad_rf=${fld:entidades} and smn_comercial.smn_pedido_cabecera.smn_centro_facturacion_id=${fld:centro_facturacion}--and smn_comercial.smn_pedido_cabecera.pca_fecha_registro=CURRENT_DATE
	${filter}
order by
		smn_comercial.smn_pedido_cabecera.pca_fecha_registro desc
