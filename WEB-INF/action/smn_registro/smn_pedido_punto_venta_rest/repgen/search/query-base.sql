select DISTINCT
	...
	i.itm_codigo ||' - '|| i.itm_nombre as smn_item_rf,
  	s.svc_codigo ||' - '|| s.svc_descripcion as smn_servicio_rf,
	...
	m.mon_codigo ||' - '|| m.mon_nombre as smn_moneda_rf,
  	t.tca_moneda_referencia ||' - '|| t.tca_tasa_cambio as smn_tasa_rf
from
...
where
	smn_comercial.smn_pedido_cabecera.pca_estatus IN ('GE','RE') 
	${filter}
