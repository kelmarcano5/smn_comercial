...
select DISTINCT
	...
	case
	   when smn_base.smn_baremos_detalle_componente.bdc_precio_ml is null then 0 else smn_comercial.smn_orden_servicio_detalle.osd_cantidad*smn_base.smn_baremos_detalle_componente.bdc_precio_ml
	end as prd_monto_moneda_local,
	case
	   when smn_base.smn_baremos_detalle_componente.bdc_monto_ma is null then 0 else smn_comercial.smn_orden_servicio_detalle.osd_cantidad*smn_base.smn_baremos_detalle_componente.bdc_monto_ma
...
select 
subq.smn_servicios_rf, subq.smn_componentes_rf, subq.cmp_porcentaje, subq.smn_forma_calculo_rf, subq.smn_tipo_componente, subq.smn_grupo_prestador_rf,
subq.smn_visible_rf, subq.smn_prestador_servicio_rf, subq.prd_cantidad, subq.prd_monto_moneda_local, subq.prd_moneda_alterna, subq.smn_item_rf,
subq.smn_moneda_rf, subq.precio, subq.precio_ma
from subq
where subq.smn_tipo_componente in('IT','HO','SE') and subq.smn_forma_calculo_rf IN('RP','PR')